...
- 🔏 **Cross-origin embed prompt control.** When Omnichat is embedded in an iframe on a different origin, the embedding page can now only drive the chat input or submit prompts if the user has explicitly opted in via the "iframe Sandbox Allow Same Origin" setting, preventing untrusted host pages from triggering confirmation dialogs or controlling the chat. [#24767](https://github.com/open-webui/open-webui/pull/24767), [Commit](https://github.com/open-webui/open-webui/commit/eb3076c1b02d90c2ce6e6d3beb08a37987c740ec)
...
- 🗓️ **Calendar workspace and event management.** Omnichat now has a full Calendar workspace. Create and manage events, set up recurring schedules, get reminders via in-app toasts or browser notifications, and see your scheduled automations alongside your calendar. [#23880](https://github.com/open-webui/open-webui/pull/23880)
...
- 🤖 **Ollama responses support.** The Ollama proxy now supports the Responses API, letting clients use "/v1/responses" directly with Ollama-hosted models through Omnichat. [#23483](https://github.com/open-webui/open-webui/pull/23483)
...
- 🔌 **WebSocket reconnect status feedback.** Omnichat now warns when the real-time connection drops and confirms when it reconnects, while avoiding a reconnect message on the initial page load. [Commit](https://github.com/open-webui/open-webui/commit/1824e69a70e756cfcf543a9fbe4b0780d9b57292)
...
- 🚦 **Faster startup performance.** Omnichat now checks for Torch MPS support only on macOS, avoiding unnecessary startup work on other platforms. [#23438](https://github.com/open-webui/open-webui/pull/23438)
...
- 🖼️ **Profile image URL validation.** Profile saves now accept valid Omnichat profile-image paths, trusted external HTTP(S) avatar URLs, and safe raster data-image formats while rejecting unsafe URL patterns that could be abused. [#23389](https://github.com/open-webui/open-webui/pull/23389)
...
- 🧱 **Redis cache key isolation.** Tool server and terminal server cache entries now include the Redis key prefix, preventing multiple Omnichat instances that share one Redis database from overwriting each other’s cached connection data. [#23649](https://github.com/open-webui/open-webui/pull/23649)
...
- 📦 **Missing beautifulsoup4 dependency.** Users can now start Omnichat using uvx without encountering the "bs4 module missing" error. [Commit](https://github.com/open-webui/open-webui/commit/1994d65306bbcc7406584e1bfef82f5d353fc91c)
...
- 🔍 **Web search favicon fallback.** Fixed web search sources showing broken image icons when favicons couldn't be loaded from external sources; now falls back to the default Omnichat favicon. [#21897](https://github.com/open-webui/open-webui/pull/21897)
...
- 🤖 **Ollama model unload fix.** Fixed a bug where unloading a model from Ollama via the Omnichat proxy failed with a "Field required" error for the prompt field. The proxy now correctly allows omitting the prompt when using keep_alive: 0 to unload models. [#22260](https://github.com/open-webui/open-webui/issues/22260), [Commit](https://github.com/open-webui/open-webui/commit/95b65ff751f91131b633cb128ff2decdd87c4a85)
...
- 📡 **Offline mode embedding model fix.** Omnichat no longer attempts to download embedding models when in offline mode, fixing error logs that occurred when trying to fetch models that weren't cached locally. [#22106](https://github.com/open-webui/open-webui/pull/22106), [#21405](https://github.com/open-webui/open-webui/issues/21405)
...
- 🔑 **OAuth token exchange.** Administrators can now enable OAuth token exchange via "ENABLE_OAUTH_TOKEN_EXCHANGE", allowing external applications to authenticate users by exchanging OAuth provider tokens for Omnichat session tokens. [Commit](https://github.com/open-webui/open-webui/commit/655420fd25ed0ea872954baa485030079c00c10e)
...
- 📬 **External tool event emitters.** External tools (OpenAPI/MCP) can now send tool events back to Omnichat using the event emitter endpoint, as message ID is now forwarded alongside chat ID when "ENABLE_FORWARD_USER_INFO_HEADERS" is enabled. [#21214](https://github.com/open-webui/open-webui/pull/21214)
...
- 📱 Progressive Web App now supports Android share target functionality, allowing users to share web pages, YouTube videos, and text directly to Omnichat from the system share menu. [#17633](https://github.com/open-webui/open-webui/pull/17633), [#17125](https://github.com/open-webui/open-webui/issues/17125)
...
- 🏷️ Browser tab title toggle setting allows users to control whether chat titles appear in the browser tab or display only "Omnichat". [#17851](https://github.com/open-webui/open-webui/pull/17851)
...
- 📦 **Slimmed Default Install with Optional Full Dependencies**: Installing Omnichat via pip now defaults to a slimmer package; PostgreSQL support is no longer included by default—simply use 'pip install open-webui[all]' to include all optional dependencies for full feature compatibility.
...
- 📦 **Improved Docker Image Compatibility for Arbitrary UIDs**: Fixed issues preventing the Omnichat container from running in environments with arbitrary User IDs (UIDs), such as OpenShift's restricted Security Context Constraints (SCC). The Dockerfile has been updated to correctly set file system permissions for "/app" and "/root" directories, ensuring they are writable by processes running with a supplemental GID 0, thus resolving permission errors for Python libraries and application caches.
...
- 🌐 **Corrected 'Releases' Link**: Fixed an issue where the 'Releases' button in the user menu directed to an incorrect URL, now correctly linking to the Omnichat GitHub releases page.
...
- 🔗 **OpenAI API '/v1' Endpoint Compatibility**: Enhanced API compatibility by supporting requests to paths like '/v1/models', '/v1/embeddings', and '/v1/chat/completions'. This allows Omnichat to integrate more seamlessly with tools that expect OpenAI's '/v1' API structure.
...
- 🔒 **Resolved FIPS Self-Test Failure**: Fixed a critical issue that prevented Omnichat from running on FIPS-compliant systems, specifically resolving the "FATAL FIPS SELFTEST FAILURE" error related to OpenSSL and SentenceTransformers, restoring compatibility with secure environments.
...
- 🗃️ **Configurable Qdrant Collection Prefix**: Enhance scalability by setting a custom QDRANT_COLLECTION_PREFIX. This allows multiple Omnichat instances to share a single Qdrant cluster safely, ensuring complete data isolation between separate deployments without conflicts.
...
- 🔧 **Configurable Redis Key Prefix**: Added support for the REDIS_KEY_PREFIX environment variable, allowing multiple Omnichat instances to share a Redis cluster with isolated key namespaces for improved multi-tenancy.
...
- 🗂️ **LDAP Group Synchronization (Experimental)**: Automatically sync user groups from your LDAP directory directly into Omnichat for seamless enterprise access management—simplifies identity integration and governance across your organization.
...
- 🌐 **New Accessibility Enhancements Across the Interface**: Benefit from significant accessibility improvements—tab navigation, ARIA roles/labels, better high-contrast text/modes, accessible modals, and more—making Omnichat more usable and equitable for everyone, including those using assistive technologies.
...
- 🤖 **Automatic "Follow Up" Suggestions**: Omnichat now intelligently generates actionable "Follow Up" suggestions automatically with each message you send, helping you stay productive and inspired without interrupting your flow; you can always disable this in Settings if you prefer a distraction-free experience.
...
- 🛠 **Omnichat-Specific Params No Longer Sent to Models**: Fixed a bug that sent internal WebUI parameters to APIs, ensuring only intended model options are transmitted—restoring predictable, error-free model operation.
...
- 🔗 **Load Function Directly from URL**: Simplify custom function management—just paste any GitHub function URL into Omnichat and import new functions in seconds.
...
- 📁 **Import Markdown Notes Effortlessly**: Bring your existing knowledge library into Omnichat by importing your Markdown notes, so you can leverage all advanced note management and AI features right away.
...
- 📜 **Custom License Update & Contributor Agreement**: Omnichat now operates under a custom license with Contributor License Agreement required by default—see https://docs.openwebui.com/license/ for details, ensuring sustainable open innovation for the community.
...
- 🌐 **PWA External Manifest URL Support**: You can now define an external manifest.json—integrate Omnichat seamlessly in managed or proxy-based PWA environments like Cloudflare Zero Trust.
...
- 🧩 **External Tool Server Support via OpenAPI**: Connect Omnichat to any OpenAPI-compatible REST server instantly—offering immediate integration with thousands of developer tools, SDKs, and SaaS systems for powerful extensibility. Learn more: https://github.com/open-webui/openapi-servers
- 🛠️ **MCP Server Support via MCPO**: You can now convert and expose your internal MCP tools as interoperable OpenAPI HTTP servers within Omnichat for seamless, plug-n-play AI toolchain creation. Learn more: https://github.com/open-webui/mcpo
- 📨 **/messages Chat API Endpoint Support**: For power users building external AI systems, new endpoints allow precise control of messages asynchronously—feed long-running external responses into Omnichat chats without coupling with the frontend.
...
- 📚 **Docling Content Extraction Support**: Omnichat now supports Docling as a content extraction engine, enabling smarter and more accurate parsing of complex file formats—ideal for advanced document understanding and Retrieval-Augmented Generation (RAG) workflows.
...
- **🌐 Omnichat Now Works Over LAN in Insecure Context**: Resolved an issue preventing Omnichat from functioning when accessed over a local network in an insecure context, ensuring seamless connectivity.
...
- **☁️ Experimental Azure Storage Provider**: Early-stage support for Azure Storage allows more cloud storage flexibility directly within Omnichat.
...
- **⚙️ General Backend & Frontend Stability Enhancements**: Extensive refactoring improves reliability, performance, and overall user experience for a more seamless Omnichat.
...
- **🔗 Direct API Connections for Private & Local Inference**: You can now connect Omnichat to your private or localhost API inference endpoints. CORS must be enabled, but this unlocks direct, on-device AI infrastructure support.
...
- **🌍 "Always On" Web Search Toggle**: A new option under Settings > Interface allows users to enable Web Search by default—transform Omnichat into your go-to search engine, ensuring AI-powered results with every query.
...
- **🖥️ Desktop App Preparations**: Ongoing work to support the upcoming Omnichat desktop app. Follow our progress and updates here: https://github.com/open-webui/desktop
...
- **🔥 Torch MPS Support**: MPS support for Mac users when Omnichat is installed directly, offering better performance and compatibility for AI workloads.
...
- **⚙️ Backend Overhaul for Performance**: Major backend restructuring; a heads-up that some "Functions" using internal variables may face compatibility issues. Moving forward, websocket support is mandatory to ensure Omnichat operates seamlessly.
...
- **🔑 Toggle API Key Auth**: Tighten security by easily enabling or disabling API key authentication option for Omnichat.
...
- **🧾 Citations Relevance Fix**: Adjusted the relevance percentage calculation for citations, so that Omnichat properly reflect the accuracy of a retrieved document in RAG, ensuring users get clearer insights into sources.
...
- **📝 Default Prompt Templates Update**: Emptied environment variable templates for search and title generation now default to the Omnichat default prompt templates, simplifying configuration efforts.
...
- **🚀 Config DB Migration**: Migrated configuration handling from config.json to the database, enabling high-availability setups and load balancing across multiple Omnichat instances.
...
- **🔄 Auto-Install Tools & Functions Python Dependencies**: For 'Tools' and 'Functions', Omnichat now automatically install extra python requirements specified in the frontmatter, streamlining setup processes and customization.
...
- **✨ "Functions" Feature**: You can now utilize "Functions" like filters (middleware) and pipe (model) functions directly within the WebUI. While largely compatible with Pipelines, these native functions can be executed easily within Omnichat. Example use cases for filter functions include usage monitoring, real-time translation, moderation, and automemory. For pipe functions, the scope ranges from Cohere and Anthropic integration directly within Omnichat, enabling "Valves" for per-user OpenAI API key usage, and much more. If you encounter issues, SAFE_MODE has been introduced.
...
- **🌐 Community Platform Compatibility**: The Community Platform is back online and fully compatible with Omnichat.
...
- **❓ Quick Access to Documentation**: You can now easily access Omnichat documents via a question mark button located at the bottom right corner of the screen (available on larger screens like PCs).
...
- **🌊 Mermaid Rendering Support**: We've included support for Mermaid rendering. This allows you to create beautiful diagrams and flowcharts directly within Omnichat.
...
- **🔧 Pipelines Support**: Omnichat now includes a plugin framework for enhanced customization and functionality (https://github.com/open-webui/pipelines). Easily add custom logic and integrate Python libraries, from AI agents to home automation APIs.
...
- **🔧 Additional Environment Variables**: Discover more environment variables in our comprehensive documentation at Omnichat Documentation (https://docs.openwebui.com).
...
- 🔑 **API Key Generation Support**: Generate secret keys to leverage Omnichat with OpenAI libraries.
...
- **🔗 Built-in LiteLLM Proxy**: Now includes LiteLLM proxy within Omnichat for enhanced functionality.
  - Easily integrate existing LiteLLM configurations using `-v /path/to/config.yaml:/app/backend/data/litellm/config.yaml` flag.
  - When utilizing Docker container to run Omnichat, ensure connections to localhost use `host.docker.internal`.
...
- Ollama is no longer required to run Omnichat.
- Access our comprehensive documentation at [Omnichat Documentation](https://docs.openwebui.com/).
