...
<script lang="ts">
	...
	let chatTitle = title.replace(/^\p{Extended_Pictographic}\s*/gu, '').trim();
	...
	const renameHandler = async () => {
		chatTitle = title.replace(/^\p{Extended_Pictographic}\s*/gu, '').trim();
		confirmEdit = true;
			...
			<div class="flex self-center flex-1 w-full min-w-0">
				...
				<div
					...
					class="text-left self-center overflow-hidden w-full h-[20px] truncate {unread
						...
						: ''}"
				>
					{title.replace(/^\p{Extended_Pictographic}\s*/gu, '').trim()}
				</div>
			</div>
